<#
.SYNOPSIS
Audits GitHub Actions workflow files for read-only governance posture.
#>
param(
...
$jsonPath = Join-Path $OutputDirectory "workflow-governance-$timestamp.json"
$mdPath = Join-Path $OutputDirectory "workflow-governance-$timestamp.md"
...
$lines = @()
$lines += "# Workflow Governance Audit"
$lines += ""
...
if ($findings.Count -eq 0) {
    $lines += "| none | none | none | No governance findings. |"
}
else {
...
Write-Host "Workflow governance JSON: $jsonPath"
Write-Host "Workflow governance Markdown: $mdPath"
...
if ($FailOnFinding -and ($criticalFindings.Count -gt 0 -or $highFindings.Count -gt 0)) {
    throw "Workflow governance audit found high or critical findings."
}
